...
const StatCard = ({ title, value, subtitle, icon: Icon, delay = 0 }) => (
    <motion.div
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay, duration: 0.5, ease: "easeOut" }}
        whileHover={{ y: -8, scale: 1.02 }}
        className="glass-card p-8 flex flex-col justify-between border-none luxury-shadow min-h-[220px] relative overflow-hidden group"
    >
        ...
        <div className="flex justify-between items-start mb-6 relative z-10">
            <div className="p-3 bg-red-50 dark:bg-red-900/10 rounded-2xl text-red-500 shadow-inner">
                <Icon size={24} />
            </div>
        ...
        <div className="relative z-10">
            <div className="text-3xl md:text-4xl font-black text-gray-900 dark:text-white mb-2 tracking-tighter">
                {value}
            </div>
...
const ResultsDisplay = ({ results, currency = 'THB' }) => {
    ...
    const stats = [
        { title: 'Daily Revenue', value: formatCurrency(results.dailyRevenue, currency), subtitle: 'Avg. per day', icon: Clock },
        { title: 'Monthly Revenue', value: formatCurrency(results.monthlyRevenue, currency), subtitle: 'Est. per month', icon: Calendar },
        { title: 'Annual Revenue', value: formatCurrency(results.annualRevenue, currency), subtitle: 'Yearly projection', icon: TrendingUp },
        { title: 'Channel Value', value: formatCurrency(results.totalChannelValue, currency), subtitle: 'Based on 2.5x annual', icon: Briefcase },
    ];
